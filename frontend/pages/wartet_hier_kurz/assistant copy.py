...
if "openai_key" not in st.session_state:
    st.session_state["openai_key"] = xds.db_get_value(
        user, "session_data", "openai_key"
    )
...
def reset_chat_name_fun(chat_name):
    ...
    if current_chat in st.session_state["history_chats"]:
        ...
        # Handle the case where current_chat is not in the list
        current_chat_index = -1  # or some default or error handling
    ...
    xds.db_set_value(
        username, "chat_table_name", "chat_name_column", new_chat_name
    )  # Assuming table and column names
    ...
    for item in [
        ...
        *initial_content_all["paras"],
        ...
        st.session_state[item + new_chat_name + "value"] = st.session_state.get(
            item + current_chat + "value", ""
        )

    # Remove old chat entry from the database
    delete_query = f"DELETE FROM chat_table_name WHERE chat_name_column = %s"
    with xds.init_connection() as conn:
        xds.run_query(conn, delete_query, (current_chat,), return_type="none")
...
def create_chat_fun(username):
    ...
    # Add to the database
    insert_query = f"INSERT INTO chat_table_name (username_column, chat_name_column) VALUES (%s, %s)"
    with xds.init_connection() as conn:
...
def delete_chat_fun(username):
    ...
    delete_query = f"DELETE FROM chat_table_name WHERE chat_name_column = %s AND username_column = %s"
    with xds.init_connection() as conn:
...
with st.sidebar:
    if ("set_chat_name" in st.session_state) and st.session_state[
        ...
        reset_chat_name_fun(
            st.session_state["set_chat_name"], st.session_state["current_chat"]
        )
...
# Load data
if current_chat:
    for key, value in xds.load_data(st.session_state["path"], current_chat).items():
        if key == "history":
...
# Check and initialize session states if they are not initialized yet
if "openai_key" not in st.session_state:
...
if st.session_state["user_input_content"] != "":
    ...
    with st.spinner("🤔"):
        try:
            ...
            if apikey := st.session_state.get(
                "openai_key", None
            ):  # Use the 'openai_key' from the session state
