...
from components.llms.constants import *
...
class PdfQA:
    ...
    def vector_db_pdf(self) -> None:
        ...
        if persist_directory and os.path.exists(persist_directory):
            ...
            ## 3. Create Embeddings and add to Milvus store
            ##TODO: Validate if self.embedding is not None
            # self.vectordb = Milvus.from_documents(documents=texts, embedding=self.embedding, persist_directory=persist_directory)

            # self.vectordb = Milvus.from_documents(documents=texts, embedding=self.embedding, persist_directory=persist_directory)
            self.vectordb = Milvus.from_documents(
                documents=texts,
